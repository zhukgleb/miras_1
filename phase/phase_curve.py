...
def import_data(path: str):
    ...
    data_u = data.loc[data["Band"] == "U"]

    return data_vis, data_b, data_v, data_r, data_u
...
def nearest_mag(target_dates, obs_dates, obs_mags):
    ...
    obs_df = pd.DataFrame({"date": obs_dates, "mag": obs_mags}).sort_values("date")

    target_df = pd.DataFrame({"date": target_dates})

    result = pd.merge_asof(
        target_df.sort_values("date"), obs_df, on="date", direction="nearest"
    )
...
if __name__ == "__main__":
    d_vis, d_b, d_v, d_r, d_u = import_data("lightcurve.txt")
    ...
    #plt.scatter(date_b, mb, label="B")
    #plt.scatter(date_v, mv, label="V")
    #plt.scatter(date_r, mr, label="R")
    #plt.legend()
