...
def correct_molecular_bands(obs_wave, obs_flux, 
                            molecular_template_wave, molecular_template_flux,
    ...
    molecular_template_interp = np.interp(obs_wave, 
    ...
    if continuum_regions is None:
        ...
        peaks, properties = find_peaks(smooth_norm, 
                                       height=0.95, 
                                       distance=len(obs_wave)//20,
                                       prominence=0.05)
        ...
        if len(peaks) > 2:
            ...
            for i in range(1, len(peak_positions)):
                if peak_positions[i] - peak_positions[i-1] < (obs_wave[-1] - obs_wave[0]) / 10:
    ...
    for iteration in range(n_iterations):
        ...
        kernel = np.ones(5) / 5
        smoothed_mask = np.convolve(new_continuum_mask.astype(float), kernel, mode='same') > 0.3
        ...
        # Сохраняем историю
        history['iterations'].append(iteration + 1)
        history['rms_continuum'].append(np.std(y_cont - continuum_poly(x_cont) if len(x_cont) > polynomial_degree else 0))
        history['molecular_depth'].append(np.mean(molecular_depth))
        
        # Проверка сходимости
        if iteration > 0:
            depth_change = abs(history['molecular_depth'][-1] - history['molecular_depth'][-2])
    ...
    if plot:
        ...
        ax.plot(obs_wave, continuum_model_final, 'r-', linewidth=2, label='Continuum model')
        
        # Отмечаем точки континуума
        cont_wave = obs_wave[continuum_mask]
        cont_flux = obs_flux[continuum_mask]
        ax.scatter(cont_wave, cont_flux, c='green', s=10, alpha=0.5, label='Continuum points')
        ...
        ax = axes[0, 1]
        ax.plot(obs_wave, molecular_template_interp, 'g-', linewidth=1.5, label='Molecular template')
        ax.set_xlabel('Wavelength')
        ...
        ax.plot(obs_wave[idx_range], normalized_flux_final[idx_range], 'b-', 
                alpha=0.5, label='Normalized')
        ax.plot(obs_wave[idx_range], molecular_template_interp[idx_range], 'g--', 
                alpha=0.7, label='Molecular template')
        ax.plot(obs_wave[idx_range], molecular_corrected[idx_range], 'r-', 
                alpha=0.7, label='Molecular corrected')
        ...
        # 6. Гистограмма
        ax = axes[2, 1]
        hist_data = normalized_flux_final[(normalized_flux_final > 0.5) & (normalized_flux_final < 1.5)]
        ax.hist(hist_data, bins=50, edgecolor='black', alpha=0.7)
        ax.axvline(x=1.0, color='r', linestyle='--', linewidth=2)
        ax.axvline(x=np.median(hist_data), color='orange', linestyle='--', 
                   label=f'Median = {np.median(hist_data):.4f}')
...
def create_molecular_template(wavelength, cross_section, 
                              column_density=None,
                              normalize=True,
                              smooth_sigma=None,
    ...
    cs_min = np.min(cross_section[cross_section > 0]) if np.any(cross_section > 0) else 1e-10
    ...
    if verbose:
        print(f"Template flux range: {np.min(template_flux):.6f} - {np.max(template_flux):.6f}")
    ...
    if smooth_sigma is not None and smooth_sigma > 0:
        template_flux = gaussian_filter1d(template_flux, sigma=smooth_sigma, mode='reflect')
        template_flux = np.clip(template_flux, 1e-10, 1.0)
        if verbose:
    ...
    if plot:
        ...
        ax.axvline(x=np.median(template_flux), color='orange', linestyle='--', 
...
def fit_column_density(obs_wave, obs_flux, 
                       wavelength_cs, cross_section,
                       continuum_regions=None,
                       n_iterations=20,
                       plot=True,
                       verbose=True):
...
def correct_spectrum_with_molecular_bands(obs_wave, obs_flux,
    ...
    for iteration in range(n_iterations):
        ...
        correction_mask = (current_flux > expected_flux * 1.05) & (molecular_template_interp < 0.95)
        
        if np.sum(correction_mask) > 0:
            if verbose:
                print(f"  Found {np.sum(correction_mask)} points with underestimated continuum")
            
            # Корректируем эти точки
            # Повышаем континуум в этих областях
            correction_factor = current_flux[correction_mask] / expected_flux[correction_mask]
            ...
            if not np.isnan(median_correction) and median_correction > 1.0:
                ...
                continuum_model[correction_mask] = continuum_model[correction_mask] * median_correction
        ...
        history['rms_continuum'].append(np.std(y_cont - continuum_poly(x_cont) if len(x_cont) > poly_degree else 0))
        ...
        # Проверка сходимости
        if iteration > 0:
            change = abs(history['median_spectrum'][-1] - history['median_spectrum'][-2])
            if change < 0.001:
                if verbose:
                    print(f"  Converged after {iteration + 1} iterations")
                break
    ...
    if plot:
        ...
        ax.scatter(cont_wave, cont_flux, c='green', s=5, alpha=0.5, label='Continuum points')
        ...
        ax = axes[1, 0]
        ax.plot(obs_wave, normalized_flux_final, 'b-', alpha=0.7, label='Normalized')
        ax.axhline(y=target_median, color='r', linestyle='--', linewidth=2, 
        ...
        ax.plot(obs_wave[idx_range], molecular_template_interp[idx_range] * target_median, 
                'g--', alpha=0.7, label='Template')
        ...
        # 5. Эволюция
        ax = axes[2, 0]
        ax.plot(history['iterations'], history['median_spectrum'], 'bo-', 
                linewidth=1.5, label='Spectrum median')
        ax.plot(history['iterations'], history['median_continuum'], 'ro-', 
                linewidth=1.5, label='Continuum median')
        ...
        # 6. Гистограмма
        ax = axes[2, 1]
        hist_data = normalized_flux_final[(normalized_flux_final > 0.3) & 
                                           (normalized_flux_final < 1.5)]
        ax.hist(hist_data, bins=50, edgecolor='black', alpha=0.7)
...
def normalize_echelle_order(obs_wave, obs_flux,
                            molecular_template_wave, molecular_template_flux,
                            order_range=None,
                            poly_degree=5,
                            n_iterations=5,
    ...
    for iteration in range(n_iterations):
        ...
        if len(x_cont) < poly_degree + 2:
            ...
            try:
                ...
                continuum_model[:edge_width] = np.clip(continuum_model[:edge_width], 
                                                       0.5 * edge_median, 2 * edge_median)
                continuum_model[-edge_width:] = np.clip(continuum_model[-edge_width:],
                                                        0.5 * edge_median, 2 * edge_median)
        ...
        # Проверка сходимости
        if iteration > 0:
            change = abs(history['spectrum_median'][-1] - history['spectrum_median'][-2])
            if change < 0.001:
                if verbose:
                    print(f"    Converged")
                break
    ...
    if cont_median > 0:
        ...
        if verbose:
            ...
            print(f"  Final continuum median: {np.median(normalized_flux_final[current_mask]):.4f}")
    ...
    if plot:
        ...
        cont_wave = obs_wave[current_mask]
        cont_flux = obs_flux[current_mask]
        ax.scatter(cont_wave, cont_flux, c='green', s=20, alpha=0.7, 
        ...
        ax.scatter(obs_wave[idx_range][cont_idx], 
                  normalized_flux_final[idx_range][cont_idx],
                  c='green', s=30, alpha=0.7, label='Continuum')
...
def normalize_all_orders(obs_wave, obs_flux,
    ...
    for i, (wl_min, wl_max) in enumerate(order_boundaries):
        if verbose:
            print(f"\n{'='*50}")
            print(f"Order {i+1}/{len(order_boundaries)}: {wl_min:.1f} - {wl_max:.1f} Å")
            print(f"{'='*50}")
...
def normalize_with_molecular_continuum_cross_section(
    model_wave, model_flux, model_diff,
    obs_wave, obs_flux,
    molecular_wave, molecular_cross_section,
    threshold=0.1,
    poly_degree=5,
    ...
    if np.max(molecular_cross_section) == 0:
        ...
        result = normalize_without_molecular_fallback(
            model_wave, model_flux, model_diff,
            obs_wave, obs_flux,
            threshold, poly_degree, n_iterations, sigma_clip, plot, verbose
        )
        return result[0], result[1], 0.0, 0.0, result[2]
    
    # === ШАГ 2: Интерполяция молекулярного шаблона ===
    molecular_cross_interp = np.interp(obs_wave, molecular_wave, molecular_cross_section, 
                                       left=0.0, right=0.0)
    
    if np.max(molecular_cross_interp) == 0:
        if verbose:
            print("\n⚠️  WARNING: No overlap between molecular and observed wavelengths!")
            print(f"   Molecular range: {molecular_wave[0]:.2f} - {molecular_wave[-1]:.2f}")
    ...
    # Сглаживание
    if smooth_molecular:
        molecular_cross_interp = gaussian_filter1d(molecular_cross_interp, 
                                                   sigma=molecular_smooth_sigma, 
                                                   mode='reflect')
    ...
    if verbose:
        ...
        print(f"  σ_min (positive): {np.min(molecular_cross_interp[nonzero_mask]):.4e} cm²")
    
    # === ШАГ 3: Интерполяция на сетку модели ===
    obs_flux_interp = np.interp(model_wave, obs_wave, obs_flux)
    molecular_cross_model = np.interp(model_wave, obs_wave, molecular_cross_interp, left=0.0, right=0.0)
    ...
    def objective_function(log10_N):
        ...
        # Вычисляем отношения
        y_ratios = obs_flux_interp[ref_indices] / (model_flux[ref_indices] * transmission_ref)
    ...
    log10_N_range = (np.log10(column_density_range[0]), np.log10(column_density_range[1]))
    ...
    for iteration in range(n_iterations):
        ...
        # Вычисляем отношения
        y_fit = current_obs_interp[ref_indices] / (model_flux[ref_indices] * transmission_model[ref_indices])
        ...
        current_diff = np.abs(current_obs_interp / (model_flux * transmission_model) - np.mean(model_flux))
    ...
    if plot:
        ...
        # 2. Column density
        ax = axes[0, 1]
        ax.text(0.1, 0.5, f'N = {column_density:.4e} cm⁻²', 
                fontsize=14, transform=ax.transAxes)
        ax.text(0.1, 0.3, f'τ_max = {optical_depth_max:.4f}', 
                fontsize=14, transform=ax.transAxes)
        ax.text(0.1, 0.1, f'τ_mean = {optical_depth_mean:.4f}', 
                fontsize=14, transform=ax.transAxes)
        ax.axis('off')
        ...
        ax.plot(obs_wave[idx_range], obs_flux[idx_range], 'b-', 
                linewidth=0.8, alpha=0.5, label='Original')
        ax.plot(obs_wave[idx_range], final_continuum[idx_range], 'r-', 
                linewidth=1.5, label='Continuum model')
        ax.set_xlabel('Wavelength')
        ax.set_ylabel('Flux')
        ...
        ax = axes[1, 1]
        ax.plot(obs_wave[idx_range], final_normalized[idx_range], 'g-', 
        ...
        N_values = np.logspace(np.log10(column_density_range[0]), 
                               np.log10(column_density_range[1]), 20)
        chi2_values = []
        for N_test in N_values:
            transmission_test = np.exp(-N_test * molecular_cross_model)
            transmission_test = np.clip(transmission_test, 1e-6, 1.0)
            y_test = obs_flux_interp[ref_indices] / (model_flux[ref_indices] * transmission_test[ref_indices])
            chi2 = np.std(y_test - 1.0)**2
            chi2_values.append(chi2)
        
        ax.plot(N_values, chi2_values, 'b-', linewidth=1.5)
        ax.axvline(x=column_density, color='r', linestyle='--', 
        ...
        idx_range_mol = slice(max(0, max_idx - half_window), 
                             min(len(obs_wave), max_idx + half_window))
        
        ax.plot(obs_wave[idx_range_mol], obs_flux[idx_range_mol], 'b-', 
                linewidth=0.8, alpha=0.5, label='Original')
        ax.plot(obs_wave[idx_range_mol], final_continuum[idx_range_mol], 'r-', 
                linewidth=1.5, label='Continuum')
        ax.plot(obs_wave[idx_range_mol], final_normalized[idx_range_mol], 'g-', 
                linewidth=0.8, alpha=0.7, label='Normalized')
        ax.axhline(y=1.0, color='gray', linestyle='--', alpha=0.5)
        ax.set_xlabel('Wavelength')
        ax.set_ylabel('Flux')
...
def normalize_without_molecular_fallback(model_wave, model_flux, model_diff,
                                         obs_wave, obs_flux,
                                         threshold, poly_degree, n_iterations, 
                                         sigma_clip, plot, verbose):
...
def plot_normalized_spectrum(obs_wave, obs_flux, normalized_flux, 
    ...
    if cont_plot is not None:
        ax1.plot(wave_plot, cont_plot, 'r-', linewidth=2, alpha=0.8, label='Continuum model')
    
    # Нормированный спектр (смещён вверх для наглядности)
    offset = 0.2 * np.max(flux_plot) if np.max(flux_plot) > 0 else 0.5
    ax1.plot(wave_plot, norm_plot + offset, 'g-', linewidth=1.2, alpha=0.8, label='Normalized (+offset)')
    ...
    if continuum_model is not None and len(axes) > 2:
        ...
        ax3.set_title(f'Residual flexure (RMS = {np.std(residuals_full):.6f})', fontsize=12)
...
def plot_full_spectrum_comparison(obs_wave, obs_flux, normalized_flux,
                                  continuum_model=None,
                                  molecular_cross=None,
                                  column_density=None,
                                  optical_depth=None,
    ...
    if continuum_model is not None:
        ax1.plot(obs_wave, continuum_model, 'r-', linewidth=1.5, alpha=0.8, label='Continuum')
    ...
    if molecular_cross is not None:
        ...
        half_window = min(500, len(obs_wave) // 6)
        idx_range = slice(max(0, max_idx - half_window), 
                         min(len(obs_wave), max_idx + half_window))
        
        ax3.plot(obs_wave[idx_range], obs_flux[idx_range], 'b-', 
                linewidth=0.8, alpha=0.5, label='Original')
        if continuum_model is not None:
            ax3.plot(obs_wave[idx_range], continuum_model[idx_range], 'r-', 
                    linewidth=1.5, label='Continuum')
        ...
        ax4.plot(obs_wave[idx_range], normalized_flux[idx_range], 'g-', 
                linewidth=0.8, alpha=0.7)
    ...
    ax6.axvline(x=np.median(hist_data), color='orange', linestyle='--', 
               label=f'Median = {np.median(hist_data):.4f}')
