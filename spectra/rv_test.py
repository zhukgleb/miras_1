...
class SpectrumShifter:
    ...
    def create_widgets(self):
        ...
        self.info_text = ax_info.text(
            0, 0.5, "", transform=ax_info.transAxes, va="center", fontsize=9
        )
    ...
    def on_press(self, event):
        ...
        if event.button == MouseButton.LEFT:
            ...
            if clicked_spectrum is not None:
                ...
                self.update_status(
                    f"Выбран: {self.spectra[self.selected_spectrum]['label']}"
                )
                self.plot_spectra()
    ...
    def plot_spectra(self):
        ...
        self.ax.set_title(
            "Кликните по спектру и двигайте мышью | ← → ↑ ↓ для точной настройки"
        )
