...
tio_spectrum_path = "/home/delta/exocross/input/ZrO_6400.xsec"
stick_path = "/home/delta/exocross/input/ZrO_6400.stick"
...
nu_tio = 10e7 / nu_tio
nu_tio_s, stick = np.loadtxt(stick_path, unpack=True)
nu_tio_s = 10e7 / nu_tio_s

# rv = 34.13 km/s ?
# array[array[:, column_index].argsort()]
tio = np.column_stack((nu_tio, sigma_tio))
tio = tio[tio[:, 0].argsort()]
tios = np.column_stack((nu_tio_s, stick))
tios = tios[tios[:, 0].argsort()]


nu_tio, sigma_tio = tio[:, 0], tio[:, 1]
nu_tio_s, stick = tios[:, 0], tios[:, 1]

# rv = -116
rv = -125
_, nu_tio = pyasl.dopplerShift(nu_tio, sigma_tio, rv, edgeHandling="firstlast")
_, nu_tio_s = pyasl.dopplerShift(nu_tio_s, stick, rv, edgeHandling="firstlast")
...
_, nu_obs = pyasl.dopplerShift(nu_obs, F_obs, -5, edgeHandling="firstlast")
...
plt.ylabel("Оптическая толщина $\\tau$")
plt.title(f"Оптическая толщина ZrO (N = {N_TiO:.1e} см$^{{-2}}$)")
plt.grid(True, alpha=0.3)
plt.yscale("log")  # Логарифмическая шкала для лучшей визуализации

# График 3: Исходный и поглощенный спектры
plt.subplot(3, 1, 3)
plt.plot(nu_tio, F_star_interp, "k-", linewidth=1.0, label="Модельный спектр звезды")
plt.plot(nu_tio, F_transmitted, "g-", linewidth=2, label="Спектр после поглощения ZrO")
# plt.plot(nu_tio, F_absorbed, "r-", linewidth=0.8, label="Поглощенный спектр ZrO")
plt.xlabel("Длина волны, АА")
...
plt.plot(nu_tio_s, stick * 10e22, color="black", label="stick spectra")
plt.xlabel("Длина волны, АА")
plt.ylabel("Поток (эрг/с/см$^2$/см$^{-1}$)")
